...
def plot_trend_and_required(df: pd.DataFrame, base_year = 2015, end_year = 2030, carg_a = None,carg_r = None, sdg = 0, target_value = None, column_name:str = 'value'):
    """
    The sign of the carg_a and the carg_r matters
    base_year (str): the year for which the CARG shall be computed

    """
    def interporlate_2015val(df:pd.DataFrame, sdg:int,column_name = 'value'):
        ...
        carg = (post_val/pre_val)**(1/(upper_year - lower_year))
        interpolated_2015val = pre_val* carg **(2015-lower_year)
    ...
    if base_year < 2015:
        try:
            ...
            val_2015 = interporlate_2015val(df, sdg, column_name)
    ...
    ax.annotate("{:.0f}".format(val_2015),(plot_base_year,val_2015),horizontalalignment = 'center',verticalalignment = 'center', color = font_color[sdg])
    # latest yest
    ax.plot(year_latest, data_latest, markersize = markersize, marker = 's', color = SDG_col[sdg])
    ax.annotate("{:.0f}".format(data_latest), (year_latest,data_latest),horizontalalignment = 'center',verticalalignment = 'center',color = font_color[sdg])

    # plot target
    ax.plot(end_year,target_value,markersize = markersize, marker = 's', color = 'lightgrey', zorder= 1)
    ax.annotate("{:.0f}".format(target_value),(end_year,target_value),horizontalalignment = 'center',verticalalignment = 'center')
    # target trajectory 
    ...
    value = df.loc[year_latest, column_name] 
    value_list = [value * (1+carg_a)**i for i in range(len(year))]
    pd.Series(value_list,index = year).plot(ax =ax , linestyle = '--', color = SDG_col[sdg])
    # annotation
    ax.plot(2030,value_list[-1],marker = 's',markersize = markersize,color = SDG_col[sdg],zorder= 3)
    ax.annotate("{:.0f}".format(value_list[-1]),(2030,value_list[-1]), zorder = 3,horizontalalignment = 'center',verticalalignment = 'center',color = font_color[sdg])
